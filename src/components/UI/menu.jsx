...
export function Menu() {
  ...
  const getClass = (path) => {
    return path === location ? "menu__item menu__item-active" : "menu__item";
  };

  return (
    <nav className="menu">
      <NavLink
        className={() => getClass(ROUTER_PATH.menu)}
        to={ROUTER_PATH.menu}
      >
        Главная
      </NavLink>
      <NavLink
        className={() => getClass(`${ROUTER_PATH.menu}/drift`)}
        to={`${ROUTER_PATH.menu}/drift`}
      >
        Дрифт-такси
      </NavLink>
      <NavLink
        className={() => getClass(`${ROUTER_PATH.menu}/timeattack`)}
        to={`${ROUTER_PATH.menu}/timeattack`}
      >
        Time Attack
      </NavLink>
      <NavLink
        className={() => getClass(`${ROUTER_PATH.menu}/forza`)}
        to={`${ROUTER_PATH.menu}/forza`}
      >
        Forza Karting
      </NavLink>
    </nav>
